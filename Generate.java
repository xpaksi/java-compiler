...
class Generate {
  ...
  void stackPush(int item, int[] stack) {
    stack[stack[0]] = item;
    stack[0] = stack[0] + 1;
    if (stack[0] > stackSize - 1) System.out.println(
  ...
  public void R(int ruleNo) {
    ...
    switch (ruleNo) {
      ...
      case 1:
        ...
        if (ll > HMachine.displaySize) System.out.println(
      ...
      case 38:
        ...
        if (!isStackEmpty(divStack)) createCheckDiv();
        if (!isStackEmpty(boundStack)) createOutOfRangeMessage();
        if (!isStackEmpty(printStack)) createPrintRoutine();
        break;
      ...
      case 44:
        ...
        int entryAddr = Context.symbolHash
          .find(Context.currentStr)
          .getCodeAddress();
      ...
      case 46:
        ...
        cell = cell + 6;
        break;
      // R47: After function call returns
      case 47:
        top = stackPop(returnAddrStack, top);
        HMachine.memory[top] = cell + 3;

        // Branch to function
        entryAddr = Context.symbolHash
          .find(Context.currentStr)
          .getCodeAddress();
        HMachine.memory[cell] = HMachine.PUSH;
        HMachine.memory[cell + 1] = entryAddr;
        ...
        // Load result from mt-1
        HMachine.memory[cell + 3] = HMachine.PUSHMT;
        HMachine.memory[cell + 4] = HMachine.PUSH;
        HMachine.memory[cell + 5] = 1;
        HMachine.memory[cell + 6] = HMachine.SUB;
        HMachine.memory[cell + 7] = HMachine.LOAD;

        // First, get address of ret_val slot: PUSHMT, PUSH 5, SUB = address of ret_val
        HMachine.memory[cell + 8] = HMachine.PUSHMT;
        HMachine.memory[cell + 9] = HMachine.PUSH;
        HMachine.memory[cell + 10] = 5;
        HMachine.memory[cell + 11] = HMachine.SUB;
        HMachine.memory[cell + 12] = HMachine.FLIP;
        HMachine.memory[cell + 13] = HMachine.STORE;
        HMachine.memory[cell + 14] = HMachine.PUSH;
        HMachine.memory[cell + 15] = 4;
        HMachine.memory[cell + 16] = HMachine.POP;
        cell = cell + 17;
        break;
      ...
      case 50:
        ...
        if (kode == Bucket.FUNCTION) {
          ...
          // Store result_copy at position of ret_val
          HMachine.memory[cell + 8] = HMachine.PUSHMT;
          HMachine.memory[cell + 9] = HMachine.PUSH;
          HMachine.memory[cell + 10] = 5;
          HMachine.memory[cell + 11] = HMachine.SUB; // addr of ret_val slot
          HMachine.memory[cell + 12] = HMachine.FLIP;
          HMachine.memory[cell + 13] = HMachine.STORE; // Store result at ret_val's position
          // Stack: [result_copy] [ret_addr] [padding] [result]
          // Pop 3 to remove ret_addr, padding, result
          HMachine.memory[cell + 14] = HMachine.PUSH;
          HMachine.memory[cell + 15] = 3;
          HMachine.memory[cell + 16] = HMachine.POP;
          // Stack: [result_copy]
          cell = cell + 17;
        } else {
