...
class Generate {
	...
	void stackPush(int item, int[] stack) {
		...
		if (stack[0] > stackSize - 1)
			System.out.println(
					"Stack overflow in code generator.");
	}
	...
	void obtainAddress() {
		...
		HMachine.memory[cell + 1] = Context.symbolHash
				.find(Context.currentStr)
				.getLexicLev();
		HMachine.memory[cell + 2] = Context.symbolHash
				.find(Context.currentStr)
				.getOrderNum();
	...
	public void R(int ruleNo) {
		...
		switch (ruleNo) {
			...
			case 6:
				...
				// Stack layout: display[ll] points past old_disp
				// ret_val is at offset -(paramCount + 5) from display
				// For 0-param functions: ret_val at -5
				int retValOffset = -2; // default for non-function expression scopes
				if (!Context.calleeStack.isEmpty()) {
					int paramCount = Context.calleeStack.peek().getParamCount();
					retValOffset = -(paramCount + 5);
				}

				HMachine.memory[cell] = HMachine.NAME;
				HMachine.memory[cell + 1] = ll;
				HMachine.memory[cell + 2] = retValOffset;
				HMachine.memory[cell + 3] = HMachine.FLIP;
			...
			case 7:
				Context.symbolHash.find(Context.currentStr).setCodeAddress(cell + 3);
			...
			case 41:
				...
				cell = cell + 29;
				break;
			// R42:Instruction to back in the end of procedure
			case 42:
				HMachine.memory[cell] = HMachine.BR;
				cell = cell + 1;
				break;
			case 43:
				// R43: Function return - load return address and branch
				ll = Context.lexicalLevel + 1;

				int paramCount = 0;
				if (!Context.calleeStack.isEmpty()) {
					paramCount = Context.calleeStack.peek().getParamCount();
				}
				int raOffsetFromMt = paramCount + 3;
				HMachine.memory[cell] = HMachine.PUSHMT;
				HMachine.memory[cell + 1] = HMachine.PUSH;
				HMachine.memory[cell + 2] = raOffsetFromMt;
				HMachine.memory[cell + 3] = HMachine.SUB;
				HMachine.memory[cell + 4] = HMachine.LOAD;
				HMachine.memory[cell + 5] = HMachine.BR;
				cell = cell + 6;
				break;

			// R44: Instruction to call a procedure
			case 44:
				top = stackPop(returnAddrStack, top);
				HMachine.memory[top] = cell + 3;

				int entryAddr = Context.calleeStack.peek().getCodeAddress();
			...
			case 46:
				...
				HMachine.memory[cell + 1] = HMachine.undefined;
				HMachine.memory[cell + 2] = HMachine.PUSH;
				HMachine.memory[cell + 3] = HMachine.undefined;
				stackPush(cell + 3, returnAddrStack);
				HMachine.memory[cell + 4] = HMachine.PUSH;
				HMachine.memory[cell + 5] = HMachine.undefined;
				cell = cell + 6;
				break;
			// R47: After function call with arguments returns
			case 47:
				top = stackPop(returnAddrStack, top);
				HMachine.memory[top] = cell + 3;

				Bucket callee = Context.calleeStack.peek();
				entryAddr = callee.getCodeAddress();
				paramCount = callee.getParamCount();

				HMachine.memory[cell] = HMachine.PUSH;
				HMachine.memory[cell + 1] = entryAddr;
				HMachine.memory[cell + 2] = HMachine.BR;

				int popCount = paramCount + 3;

				HMachine.memory[cell + 3] = HMachine.PUSH;
				HMachine.memory[cell + 4] = popCount;
				HMachine.memory[cell + 5] = HMachine.POP;

				cell = cell + 6;
				break;
			// R48: Store argument for procedure/function call
			case 48:
				HMachine.memory[cell] = HMachine.FLIP;
				cell = cell + 1;
				break;
			...
			case 49:
				kode = Context.calleeStack.peek().getIdKind();
				if (kode == Bucket.FUNCTION) {
					...
					callee = Context.calleeStack.peek();
					HMachine.memory[cell] = HMachine.NAME;
					HMachine.memory[cell + 1] = callee.getLexicLev();
					HMachine.memory[cell + 2] = callee.getOrderNum();
					cell = cell + 3;
				}
			...
			case 50:
				...
				if (kode == Bucket.FUNCTION) {
					...
					entryAddr = Context.calleeStack.peek().getCodeAddress();
					HMachine.memory[cell] = HMachine.PUSH;
					HMachine.memory[cell + 1] = entryAddr;
					HMachine.memory[cell + 2] = HMachine.BR;

					HMachine.memory[cell + 3] = HMachine.PUSH;
					HMachine.memory[cell + 4] = 3;
					HMachine.memory[cell + 5] = HMachine.POP;
					cell = cell + 6;
				} else {
					HMachine.memory[cell] = HMachine.LOAD;
