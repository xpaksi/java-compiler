	...
	public void C(int ruleNo) {
		...
		switch (ruleNo) {
			...
			case 5:
				symbolHash
						.find(currentStr)
						.setIdType(((Integer) typeStack.peek()).intValue());
				break;
			case 6:
			...
			case 15:
				...
				if ((temp != Bucket.INTEGER) &&
						((Integer) typeStack.peek()).intValue() != Bucket.INTEGER) {
					System.out.println(
							"Unmatched type at line " + currentLine + ": " + currentStr);
					errorCount++;
				}
			...
			case 22:
				symbolHash
						.find((String) symbolStack.peek())
						.setLLON(lexicalLevel, orderNumber);
				break;
			case 23:
				symbolHash
						.find((String) symbolStack.peek())
			...
			case 24:
				if (symbolHash.isExist(currentStr, lexicalLevel)) {
					System.out.println(
							"Procedure already declared at line " +
									currentLine +
									": " +
									currentStr);
					errorCount++;
			...
			case 36:
				temp = symbolHash.find((String) symbolStack.peek()).getIdType();
				int funcTypeTemp = ((Integer) typeStack.peek()).intValue();
				if (temp != funcTypeTemp) {
					System.out.println(
